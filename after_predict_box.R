...
after_predict_allbox_model <- function(dataset_file, pred_file, target){
...
# 数据降维
  ## 数值特征降维
    # 部分数据
    some_dataset <- dataset[c("length","before_want","fir_want","before_trans","fir_trans","before_comment","fir_comment","before_zan","fir_zan","score")]
    # str(some_dataset)
    # 碎石图
    library(psych)
    ...
    pc_5 <- principal(some_dataset, nfactors = 3, scores = TRUE)
    print(pc_5)
    head(pc_5$scores)
    print(pc_5$weights)
    ...
    min_rc3 <- summary(pc_5$scores[,3])[1]
    ...
    positive_rc3 <- pc_5$scores[,3] - min_rc3 + 1
    # positive_rc2

    bc_RC1_result <- my_independent_boxcox(positive_rc1, "RC")
    bc_RC2_result <- my_independent_boxcox(positive_rc2, "RC")
    bc_RC3_result <- my_independent_boxcox(positive_rc3, "RC")
    dataset$bc_RC1 <- bc_RC1_result[[1]]
    dataset$bc_RC2 <- bc_RC2_result[[1]]
    dataset$bc_RC3 <- bc_RC3_result[[1]]


    # 数据转存与重读取
    # 转存数据
    write.csv(dataset, "bc_data_2.csv",row.names = FALSE)
    # 重新读取
    re_load_dataset_1 <- read.table("bc_data_2.csv",header=T, sep=",", fileEncoding="GBK")
    ...
    sub_bc_dataset <- cbind(dataset["bc_newbox"], sub_bc_dataset_1, dataset["bc_RC1"], dataset["bc_RC2"], dataset["bc_RC3"], sub_bc_dataset_2, sub_bc_dataset_3)
    # str(sub_bc_dataset)
    ...
    for (i in seq(1, length(type_name_list))){
        names(sub_bc_dataset)[i+6] <- type_name_list[i]
    }

    # 修改档期列名
    schedule_name_list <- c("国庆档","贺岁档","暑期档","五一档")
    for (i in seq(1, length(schedule_name_list))){
        names(sub_bc_dataset)[i+22] <- schedule_name_list[i]
    }

    # str(sub_bc_dataset)

    # 档期降维
    time_pc <- principal(sub_bc_dataset[23:26], nfactors = 3, scores = TRUE)
...
# 回归分析
    fit3 <- lm(bc_newbox~bc_RC1 + bc_RC2 + bc_RC3 + famous_actor + famous_director + 
          schedule_pc +
          爱情 + 动画 + 动作 + 犯罪 + 古装 +
          纪录片 + 家庭 + 剧情 + 科幻 + 恐怖 +
    ...
    # 测试集测试
    predict_test_2(fit3, sub_bc_test, sub_bc_test$bc_newbox)


    # 计算模型系数
    get_model_coef_2(fit3, pc_5, time_pc)
